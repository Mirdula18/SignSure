...
beforeEach(() => {
  // A unit test has no server, so the session request never settles unless a test says it
  // does. Every screen still renders; anything that needs a token simply waits.
...
describe('App session', () => {
  it('asks for no session on the home page, only once there is a document to analyse', async () => {
    // A session costs one of ten per ten minutes per address, so a visitor who only reads the
    // home page should never spend one.
    api.createSession.mockReturnValue(new Promise(() => undefined));
    const user = userEvent.setup();
    renderApp();
  ...
  it('survives a reader who presses Analyse before the session has arrived', async () => {
    // The request takes a moment. The gate used to live on the upload screen, so choosing the
    // sample straight away tore it down and the report waited for a session forever.
    let grantSession: (value: { token: string; expiresAt: number }) => void = () => undefined;
    api.createSession.mockReturnValue(
      new Promise((resolve) => {
        grantSession = resolve;
      }),
    );
    ...
    // One request, carried across the move from concerns to report rather than restarted.
    expect(api.createSession).toHaveBeenCalledTimes(1);

    act(() => {
      grantSession({ token: 'session-token', expiresAt: 1 });
    });
    await waitFor(() => {
